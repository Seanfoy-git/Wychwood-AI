...
description: "Expanding to multiple sites? A guide to the operational model, standards, and leadership structure that make multi-site growth manageable."
...
Site-level P&L requires site-level metric definitions — consistent definitions of revenue, margin, and the operational drivers that sit underneath them, applied identically across every location. This is the foundational data work that makes the rest of the multi-site model possible — the kind of [operational systems](/services/operational-systems/) implementation that has to precede expansion.
